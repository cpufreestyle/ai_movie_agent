...
def _run_abc(agent, topic, done):
    """A/B/C：素材层 + 创意层（A 阶段采集 → B 入库 → C 企划并落到 bible）。"""
    print("\n=== A 阶段：资料采集 ===")
...
def _pick_final_src() -> str:
    """选最终成片源：优先旁白同步版 ep3_vo.mp4，其次 ltx25_film.mp4。"""
    vo = os.path.join(WORKDIR, "ep3_vo.mp4")
    return vo if os.path.exists(vo) else os.path.join(WORKDIR, "ltx25_film.mp4")


def _run_publish(agent, result, done):
    """H 阶段可选投稿：投稿前体检门禁 + 失败不推翻已生成的成片。"""
    if not (load_config().get("publish", {}) or {}).get("enabled"):
        return
    ...
    if not allow:
        ...
        result["steps"].append({"step": "H 投稿跳过", "reason": "preflight 未通过"})
        return
    try:
...
def api_run_full():
    ...
    def _job():
        ...
        if do_research:
            _run_abc(agent, topic, done)

        _run_bridge(regen_sb, done)
        ...
        _run_publish(agent, result, done)
